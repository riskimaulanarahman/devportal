...
class MemorandumController extends Controller
{
    ...
    //         // Pastikan data `contractList` ada dan berbentuk array
    //         if (!isset($requestData['contractList']) || !is_array($requestData['contractList'])) {
    //             return response()->json([
    //                 "status" => "error",
    //                 "message" => "ContractList data is required and should be an array."
    //             ]);
    //         }

    //         foreach ($requestData['contractList'] as $contract) {
    //             // Ambil hanya field yang dikirim dalam request
    //             $updateData = array_filter($contract, function ($value) {
    //                 return $value !== null;
    //             });

    //             // Jika ada field untuk diperbarui, lakukan update
    //             if (!empty($updateData)) {
    ...
    //         return response()->json([
    //             "status" => "success",
    //             "message" => "ContractList updated successfully"
    //         ]);
...
//             $Worksheet->Range("C36")->Value = $data->remarks ?? ''; 
//             $Worksheet->Range("C63")->Value = $data->DesignationName ?? ''; 
//             $Worksheet->Range("A56")->Value = $data->fullname ?? ''; 
